...
func NewPeerGroup(c *config.PeerGroup) *PeerGroup {    fmt.Printf("DEJDEJ id:",271)
...
func (pg *PeerGroup) AddMember(c config.Neighbor) {    fmt.Printf("DEJDEJ id:",272)

	pg.members[c.State.NeighborAddress] = c
}

func (pg *PeerGroup) DeleteMember(c config.Neighbor) {    fmt.Printf("DEJDEJ id:",273)

	delete(pg.members, c.State.NeighborAddress)
}

func (pg *PeerGroup) AddDynamicNeighbor(c *config.DynamicNeighbor) {    fmt.Printf("DEJDEJ id:",274)

	pg.dynamicNeighbors[c.Config.Prefix] = c
}

func newDynamicPeer(g *config.Global, neighborAddress string, pg *config.PeerGroup, loc *table.TableManager, policy *table.RoutingPolicy) *Peer {    fmt.Printf("DEJDEJ id:",275)
...
func NewPeer(g *config.Global, conf *config.Neighbor, loc *table.TableManager, policy *table.RoutingPolicy) *Peer {    fmt.Printf("DEJDEJ id:",276)
...
func (peer *Peer) AS() uint32 {    fmt.Printf("DEJDEJ id:",277)

	return peer.fsm.pConf.State.PeerAs
}

func (peer *Peer) ID() string {    fmt.Printf("DEJDEJ id:",278)

	return peer.fsm.pConf.State.NeighborAddress
}

func (peer *Peer) TableID() string {    fmt.Printf("DEJDEJ id:",279)

	return peer.tableId
}

func (peer *Peer) isIBGPPeer() bool {    fmt.Printf("DEJDEJ id:",280)

	return peer.fsm.pConf.State.PeerAs == peer.fsm.gConf.Config.As
}

func (peer *Peer) isRouteServerClient() bool {    fmt.Printf("DEJDEJ id:",281)

	return peer.fsm.pConf.RouteServer.Config.RouteServerClient
}

func (peer *Peer) isRouteReflectorClient() bool {    fmt.Printf("DEJDEJ id:",282)

	return peer.fsm.pConf.RouteReflector.Config.RouteReflectorClient
}

func (peer *Peer) isGracefulRestartEnabled() bool {    fmt.Printf("DEJDEJ id:",283)

	return peer.fsm.pConf.GracefulRestart.State.Enabled
}

func (peer *Peer) getAddPathMode(family bgp.RouteFamily) bgp.BGPAddPathMode {    fmt.Printf("DEJDEJ id:",284)
...
func (peer *Peer) isAddPathReceiveEnabled(family bgp.RouteFamily) bool {    fmt.Printf("DEJDEJ id:",285)

	return (peer.getAddPathMode(family) & bgp.BGP_ADD_PATH_RECEIVE) > 0
}

func (peer *Peer) isAddPathSendEnabled(family bgp.RouteFamily) bool {    fmt.Printf("DEJDEJ id:",286)

	return (peer.getAddPathMode(family) & bgp.BGP_ADD_PATH_SEND) > 0
}

func (peer *Peer) isDynamicNeighbor() bool {    fmt.Printf("DEJDEJ id:",287)

	return peer.fsm.pConf.Config.NeighborAddress == "" && peer.fsm.pConf.Config.NeighborInterface == ""
}

func (peer *Peer) recvedAllEOR() bool {    fmt.Printf("DEJDEJ id:",288)
...
func (peer *Peer) configuredRFlist() []bgp.RouteFamily {    fmt.Printf("DEJDEJ id:",289)

	rfs, _ := config.AfiSafis(peer.fsm.pConf.AfiSafis).ToRfList()
	return rfs
}

func (peer *Peer) toGlobalFamilies(families []bgp.RouteFamily) []bgp.RouteFamily {    fmt.Printf("DEJDEJ id:",290)
...
func classifyFamilies(all, part []bgp.RouteFamily) ([]bgp.RouteFamily, []bgp.RouteFamily) {    fmt.Printf("DEJDEJ id:",291)
...
func (peer *Peer) forwardingPreservedFamilies() ([]bgp.RouteFamily, []bgp.RouteFamily) {    fmt.Printf("DEJDEJ id:",292)
...
func (peer *Peer) llgrFamilies() ([]bgp.RouteFamily, []bgp.RouteFamily) {    fmt.Printf("DEJDEJ id:",293)
...
func (peer *Peer) isLLGREnabledFamily(family bgp.RouteFamily) bool {    fmt.Printf("DEJDEJ id:",294)
...
func (peer *Peer) llgrRestartTime(family bgp.RouteFamily) uint32 {    fmt.Printf("DEJDEJ id:",295)
...
func (peer *Peer) llgrRestartTimerExpired(family bgp.RouteFamily) bool {    fmt.Printf("DEJDEJ id:",296)
...
func (peer *Peer) markLLGRStale(fs []bgp.RouteFamily) []*table.Path {    fmt.Printf("DEJDEJ id:",297)
...
func (peer *Peer) stopPeerRestarting() {    fmt.Printf("DEJDEJ id:",298)
...
func (peer *Peer) getAccepted(rfList []bgp.RouteFamily) []*table.Path {    fmt.Printf("DEJDEJ id:",299)

	return peer.adjRibIn.PathList(rfList, true)
}

func (peer *Peer) filterpath(path, old *table.Path) *table.Path {    fmt.Printf("DEJDEJ id:",300)
...
func (peer *Peer) getBestFromLocal(rfList []bgp.RouteFamily) ([]*table.Path, []*table.Path) {    fmt.Printf("DEJDEJ id:",301)
...
func (peer *Peer) processOutgoingPaths(paths, olds []*table.Path) []*table.Path {    fmt.Printf("DEJDEJ id:",302)
...
func (peer *Peer) handleRouteRefresh(e *FsmMsg) []*table.Path {    fmt.Printf("DEJDEJ id:",303)
...
func (peer *Peer) doPrefixLimit(k bgp.RouteFamily, c *config.PrefixLimitConfig) *bgp.BGPMessage {    fmt.Printf("DEJDEJ id:",304)
...
func (peer *Peer) updatePrefixLimitConfig(c []config.AfiSafi) error {    fmt.Printf("DEJDEJ id:",305)
...
func (peer *Peer) handleUpdate(e *FsmMsg) ([]*table.Path, []bgp.RouteFamily, *bgp.BGPMessage) {    fmt.Printf("DEJDEJ id:",306)
...
func (peer *Peer) startFSMHandler(incoming *channels.InfiniteChannel, stateCh chan *FsmMsg) {    fmt.Printf("DEJDEJ id:",307)

	peer.fsm.h = NewFSMHandler(peer.fsm, incoming, stateCh, peer.outgoing)
}

func (peer *Peer) StaleAll(rfList []bgp.RouteFamily) {    fmt.Printf("DEJDEJ id:",308)

	peer.adjRibIn.StaleAll(rfList)
}

func (peer *Peer) PassConn(conn *net.TCPConn) {    fmt.Printf("DEJDEJ id:",309)
...
func (peer *Peer) ToConfig(getAdvertised bool) *config.Neighbor {    fmt.Printf("DEJDEJ id:",310)
...
func (peer *Peer) DropAll(rfList []bgp.RouteFamily) {    fmt.Printf("DEJDEJ id:",311)

	peer.adjRibIn.Drop(rfList)
}

func (peer *Peer) stopFSM() error {    fmt.Printf("DEJDEJ id:",312)
